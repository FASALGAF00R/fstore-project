  ...
  try {
    ...
    if (userData) {
      ...
      if (passwordMatch) {
        ...
        res.render('user/login', { message: "Incorrect email and password" });
      }
    } else {
...
const insertuser = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      setTimeout(async () => {
        const userWithExpiredOTP = await user.findOneAndUpdate({ _id: userData._id, otp: otp }, { $unset: { otp: '', expiry: '' } }, { new: true });
...
const loadwishlist = async (req, res) => {
  try {
    ...
    if (!wishlist) {
      // Handle the case when wishlist is not found for the user
      const products = [];
      const count = 0;
      res.render('user/wishlist', { products, count, userCart, wishlist });
    } else {
    ...
    res.render('error-page', { error: 'Error loading wishlist.' });
  }
};
...
const loadcheckout = async (req, res) => {
  try {
    ...
    console.log("load checkout", error.message);
  }
};
